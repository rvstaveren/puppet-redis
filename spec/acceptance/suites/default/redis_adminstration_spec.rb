...
describe 'redis::administration', unless: default['hypervisor'] =~ %r{docker} do
  ...
  specify do
    expect(file('/proc/sys/vm/overcommit_memory')).to have_attributes(content: "1\n")
  end

  specify do
    expect(file('/proc/sys/net/core/somaxconn')).to have_attributes(content: "65535\n")
  end

  specify do
    expect(command("timeout 1s #{redis}-server --port 7777 --loglevel verbose")).to execute_without_warning.and have_attributes(exit_status: 124)
  end
end
